...
도커파일(dockerfile)로 이미지를 빌드해서 생성한다. 
이번에는 특정 R 버전(3.3.2)을 가지고 기본 `rocker` 이미지부터 차근차근 시작해 나간다.
태그는 개발자가 이미지에 버전 번호를 붙여 활용가능하게 된다.
`rocker/verse` 이미지에 대한 [전체 버전](https://hub.docker.com/r/rocker/verse/tags/)을 참조한다.
재현가능한 분석을 위해서도 버젼 태그가 매우 유용하다.

``` {r docker-share-base, eval=FALSE}
...
``` {r docker-share-package, eval=FALSE}
...
``` {r docker-share-r-analysis, eval=FALSE}
library(ggplot2)
library(gapminder)
...
``` {r docker-share-docker-hub, eval=FALSE}
...
``` {r docker-share-image-check, eval=FALSE}
docker images
```

그리고 나면, 다음과 유사한 출력결과가 화면에 뿌려지게 된다.

``` {r docker-share-image-id, eval=FALSE}
...
``` {r docker-share-pull, eval=FALSE}
